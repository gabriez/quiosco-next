...
const ResumenProduct = ({product}) => {
    const {imagen, nombre, quantity, precio, id} = product;
    const {handleEditQuantity, handleDeleteProduct} = useQuiosco();
    ...
    return (
        <div className="shadow p-5 mb-3 flex gap-10 items-center">
            ...
            <div className="md:w-4/6">
                ...
                <p className="text-xl font-bold text-amber-500 mt-2">Precio por unidad: {formatMoney(precio)}</p>
                <p className="text-lg text-gray-700 mt-2">Subtotal: {formatMoney(precio * quantity)}</p>
            </div>
            <div>
                <button
                    type="button"
